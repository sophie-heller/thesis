    ...
    def vector_solve(self, num_steps, 
     ...
     # Iterate over k time steps
        for k in range(num_steps):
                ...
                #T_new = np.copy(T_old)                  # Initiate vector T_new (T_k+1)
            # T_old is either the initial temperature (iteration 0) or the result of the new temperature of the last time step, which becomes the old temperature of the current time step
          ...
          ###############################################################################
          # The effects caused by fast buoyancy are modeled by assuming that the physical position of the inputs may vary as the input is homogenously distributed along the tank if buoyancy conditions are met.
