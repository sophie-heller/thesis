import numpy as np
import pandas as pd
from functions_seeweg import assign_and_interpolate_layers, process_mdot_normal, process_qdot_normal
...
heights_schichtlader = [1.419, 
                        1.112, 
                        0.825, 
                        0.528]                                                     # posiiton of the openings of the Schichtlader
heights_sensors = [1.643, 
                   1.243, 
                   1.043, 
                   0.728, 
                   0.278, 
                   0.153]                                            # ** position og the 6 installed sensors in Seeweg
...
l_z = [heights_schichtlader] # [heights_schichtlader] or [0]-> no Schichtlader
...
e_z = [heights_positions_tank[2]] # [heights_positions_tank[n]] or [0] -> no heat exhanger
...
path_csv = "\\Buildings\\seeweg copy\\connections_seeweg"
...
c1 = [layernumber_c[0], pd.read_csv(f"{path}{path_csv}\\c1.csv",    #WW VL
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c2 = [layernumber_c[1], pd.read_csv(f"{path}{path_csv}\\c2.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c3 = [layernumber_c[2], pd.read_csv(f"{path}{path_csv}\\c3.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c4 = [layernumber_c[3], pd.read_csv(f"{path}{path_csv}\\c4.csv",    # WW RL1
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c5 = [layernumber_c[4], pd.read_csv(f"{path}{path_csv}\\c5.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c6 = [layernumber_c[5], pd.read_csv(f"{path}{path_csv}\\c6.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c7 = [layernumber_c[6], pd.read_csv(f"{path}{path_csv}\\c7.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c8 = [layernumber_l, pd.read_csv(f"{path}{path_csv}\\c8.csv",    # schichtlader!
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c9 = [layernumber_c[7], pd.read_csv(f"{path}{path_csv}\\c9.csv",    # WW RL2
                 parse_dates=['timestamp'],
                 index_col='timestamp')]

c10 = [layernumber_c[8], pd.read_csv(f"{path}{path_csv}\\c10.csv",    
                 parse_dates=['timestamp'],
                 index_col='timestamp')]
...
mdot_normal = [c1, c2, c3, c4, c5, c6, c7, c9, c10] # exclude connection with Schichtlader, this will be handled in main code (each step must be evaluated to determine layer). The empty space for this assignation is created by process_mdot_normal :)
...
num_steps = 1000                   # number of time steps to be made
